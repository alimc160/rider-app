<template>
  <div>
    ...
    >
      ...
      <span class="modal__title">Add company</span>
...
export default {
  ...
  methods: {
    ...
    addItem() {
      axios.post('/api/admin/vehicle_companies', this.form_data
      ).then(response => {
        ...
        this.closeModal();
        Emitter.emit("UPDATE-VEHICLE-COMPANY-DATA",response.data.data);
      }).catch(error => {
  ...
  created() {
    Emitter.on("ADD-VEHICLE-COMPANY-MODAL", (data) => {
      this.showModal = true;
    });
  },

  // destroyed() {
  //   Emitter.off('ADD-VEHICLE-COMPANY-MODAL');
  // }
}
</script>
